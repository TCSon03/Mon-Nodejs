...
app.get("/list", (req, res, next) => {
  ...
  db.query(sql, (err, data) => {
    ...
    if (err) throw err;
    res.render("admin/list", { pros: data }); //trả data về giao diện list.ejs
  });
});
...
app.get("/edit/:id", (req, res) => {
  ...
  db.query("SELECT * FROM products WHERE id=?", id, (err, data) => {
    if (err) throw err;
    res.render("admin/edit", { product: data[0] });
  });
});
...
app.post("/update/:id", upload.single("image"), (req, res) => {
  ...
  if (req.file) {
    ...
    // Nếu không có ảnh mới, chỉ cập nhật tên và giá sản phẩm
    db.query(
